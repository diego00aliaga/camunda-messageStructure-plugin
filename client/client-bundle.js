...
function Comments(eventBus, overlays, bpmnjs) {
  ...
  function toggleCollapse(element) {
    ...
    if ($overlay) {
      ...
      if (expanded) {
        ...
        $overlay.find('[data-json-editor]').focus();
      }
    }
  }
  ...
  function createCommentBox(element) {
    ...
    var $commentCount = $overlay.find('[data-comment-count]'),
        ...
        $editor = $overlay.find('[data-json-editor]'),
        $addBtn = $overlay.find('[data-add-data-field]'),
        $addRefBtn = $overlay.find('[data-add-reference-field]'),
        $fields = $overlay.find('[data-fields]'),
        $fieldTitles = $overlay.find('[data-field-titles]'),
        $saveBtn = $overlay.find('[data-save]'),
        $clearBtn = $overlay.find('[data-clear]'),
    ...
    function getPoolName(element) {
      try {
        ...
        console.log('=== DEBUGGING POOL NAME ===');
        console.log('Element:', element);
        console.log('Business Object:', bo);
        console.log('Element type:', bo.$type);
        console.log('Element name:', bo.name);
        console.log('Element ID:', bo.id);
        
        // Estrategia 1: Buscar en el contexto directo del elemento
        if (bo.lane) {
          console.log('Found lane:', bo.lane.name);
          return bo.lane.name || 'Unknown Lane';
        }
        
        if (bo.participant) {
          console.log('Found participant:', bo.participant.name);
          return bo.participant.name || 'Unknown Participant';
        }
        
        // Estrategia 2: Buscar en el proceso padre
        var process = bo.$parent;
        console.log('Direct parent:', process);
        
        while (process && process.$type !== 'bpmn:Process') {
          console.log('Parent type:', process.$type, 'Parent name:', process.name);
        ...
        if (process) {
          ...
          // Buscar la lane que contiene este proceso
          var collaboration = process.$parent;
          if (collaboration && collaboration.$type === 'bpmn:Collaboration') {
            console.log('Found collaboration:', collaboration);
            var participants = collaboration.participants || [];
            console.log('Participants:', participants);
            
			return process.id || 'Unknown Pool';

          }
          
          // Si no hay collaboration, el proceso mismo puede ser la pool
		  return process.id || 'Unknown Pool';
        }
        
        // Estrategia 3: Buscar en el diagrama completo
        var diagram = bo;
        while (diagram && diagram.$type !== 'bpmn:Collaboration') {
          diagram = diagram.$parent;
        }
        
        if (diagram) {
          console.log('Found collaboration:', diagram);
          var participants = diagram.participants || [];
          if (participants.length > 0) {
            // Buscar el participant que contiene este elemento
            for (var i = 0; i < participants.length; i++) {
              var participant = participants[i];
              if (participant.processRef) {
                var processRef = participant.processRef;
                // Verificar si este elemento está en este proceso
                if (isElementInProcess(bo, processRef)) {
                  console.log('Found participant containing element:', participant.name);
                  return participant.name || 'Unknown Pool';
                }
              }
            }
          }
        ...
        // Estrategia 4: Buscar en el contexto del elemento visual
        try {
          var elementRegistry = bpmnjs.get('elementRegistry');
          if (elementRegistry) {
            var elementId = element.id;
            var bpmnElement = elementRegistry.get(elementId);
            if (bpmnElement) {
              console.log('Found element in registry:', bpmnElement);
              // Buscar el elemento padre más cercano que sea una lane o participant
              var parent = bpmnElement.parent;
              while (parent) {
                if (parent.type === 'bpmn:Lane' || parent.type === 'bpmn:Participant') {
                  console.log('Found parent in registry:', parent);
                  return parent.businessObject.name || 'Unknown Pool';
                }
          ...
          console.log('Registry error:', registryError);
        }
        
        // Estrategia 5: Buscar en el contexto del overlay
        try {
          var overlayElement = element;
          var parentElement = overlayElement.parentNode;
          while (parentElement) {
            if (parentElement.classList && parentElement.classList.contains('djs-element')) {
              var bpmnElement = parentElement;
              if (bpmnElement.businessObject) {
                console.log('Found BPMN element in overlay context:', bpmnElement.businessObject);
                // Buscar el proceso padre
                var processParent = bpmnElement.businessObject.$parent;
                while (processParent && processParent.$type !== 'bpmn:Process') {
                  processParent = processParent.$parent;
                }
                if (processParent) {
                  console.log('Found process in overlay context:', processParent.name);
                  return processParent.name || 'Unknown Process';
                }
              }
            }
          ...
          console.log('Overlay context error:', overlayError);
        }
        
        // Estrategia 6: Buscar en el diagrama completo usando el contexto global
        try {
          if (typeof bpmnjs !== 'undefined') {
            ...
            if (elementRegistry && canvas) {
              ...
              if (rootElement && rootElement.businessObject) {
                var collaboration = rootElement.businessObject;
                if (collaboration.$type === 'bpmn:Collaboration') {
                  console.log('Found collaboration in root:', collaboration);
                  var participants = collaboration.participants || [];
                  ...
                  for (var i = 0; i < participants.length; i++) {
                    var participant = participants[i];
                    if (participant.processRef) {
                      console.log('Checking participant:', participant.name, 'Process:', participant.processRef.name);
                      // Verificar si este elemento está en este proceso
                      if (isElementInProcess(bo, participant.processRef)) {
                        console.log('Found participant containing element:', participant.name);
                        return participant.name || 'Unknown Pool';
                      }
                    }
          ...
          console.log('Global context error:', globalError);
        }
        
        console.log('Could not determine pool name, returning default');
        return 'Unknown Pool';
      } catch (e) {
        console.log('Error getting pool name:', e);
        return 'Unknown Pool';
      }
    }
    
    function isElementInProcess(element, process) {
      try {
        var current = element;
        while (current) {
          if (current === process) {
    ...
    // Función alternativa más simple para obtener el nombre de la pool
    function getPoolNameSimple(element) {
      try {
        // Intentar obtener el nombre de la pool desde el contexto del overlay
        var overlay = element.closest('.comments-overlay');
        if (overlay) {
          var bpmnElement = overlay.querySelector('.djs-element');
          if (bpmnElement && bpmnElement.businessObject) {
            var bo = bpmnElement.businessObject;
            console.log('Found BPMN element in overlay:', bo);
            ...
            if (process) {
              console.log('Found process in overlay:', process.name);
        ...
        console.log('Simple pool name error:', e);
        return 'Unknown Pool';
      }
    }

        function serializeFieldsToJSON() {
      var poolName = getPoolName(element);
      
      // Si la función principal no funciona, usar la función simple
      if (poolName === 'Unknown Pool') {
        console.log('Trying simple pool name function...');
        poolName = getPoolNameSimple(element);
      }
    ...
    function saveFieldsToBPMN(jsonData) {
      try {
        var bo = element.businessObject;
        
        if (!bo) {
          console.error('No business object found');
          return;
        }
        
        console.log('Saving fields to BPMN for element:', bo);
        
        // USAR EL MÉTODO CORRECTO PARA bpmn:documentation SEGÚN ESTÁNDAR BPMN
        try {
          if (bpmnjs && bpmnjs.get) {
            var modeling = bpmnjs.get('modeling');
            
            // bpmn:documentation va DIRECTAMENTE en el businessObject, NO en extensionElements
            var docs = bo.get('documentation') || [];
            
            // Buscar si ya existe documentación para dataFields
            ...
            // Crear nueva documentación o actualizar existente
            if (!existingDoc) {
              console.log('Creating new bpmn:Documentation in documentation array');
              var newDoc = bo.$model.create('bpmn:Documentation', {
                textFormat: 'text/plain',
                text: 'dataFields:' + JSON.stringify(jsonData)
              });
              docs.push(newDoc);
            } else {
              console.log('Updating existing bpmn:Documentation');
              existingDoc.text = 'dataFields:' + JSON.stringify(jsonData);
            }
            
            // USAR EL COMANDO OFICIAL updateProperties PARA FORZAR PERSISTENCIA
            modeling.updateProperties(element, {
              documentation: docs
            });
            
            console.log('✅ Used official modeling.updateProperties with documentation array - this SHOULD persist!');
            console.log('Fields saved to BPMN model:', jsonData);
            console.log('Documentation array:', docs);
            
            return true; // Éxito
          }
        } catch (modelingError) {
          console.error('Failed to use official modeling commands, falling back to direct method:', modelingError);
        }
        
        // MÉTODO DE RESPALDO - DIRECTO AL BUSINESS OBJECT
        console.log('Using fallback direct method...');
        
        // Crear o actualizar la extensión para los campos
        if (!bo.extensionElements) {
          console.log('Creating new extension elements');
          bo.extensionElements = bo.$model.create('bpmn:ExtensionElements');
        }
        
        // Verificar que extensionElements.values existe
        if (!bo.extensionElements.values) {
          console.log('Creating values array for extension elements');
          bo.extensionElements.values = [];
        }
        
        // Buscar si ya existe una extensión para los campos
        var existingExtension = null;
        for (var i = 0; i < bo.extensionElements.values.length; i++) {
          var ext = bo.extensionElements.values[i];
          if (ext && ext.$type === 'bpmn:Documentation') {
            if (ext.text && ext.text.includes('dataFields:')) {
        ...
        // Si no existe, crear una nueva extensión usando bpmn:Documentation
        if (!existingExtension) {
          console.log('Creating new bpmn:Documentation extension');
          existingExtension = bo.$model.create('bpmn:Documentation');
          existingExtension.text = 'dataFields:' + JSON.stringify(jsonData);
          bo.extensionElements.values.push(existingExtension);
        } else {
          console.log('Updating existing bpmn:Documentation extension');
          existingExtension.text = 'dataFields:' + JSON.stringify(jsonData);
        }
        
        console.log('Fields saved to BPMN model (fallback):', jsonData);
        console.log('Extension elements structure:', bo.extensionElements);
        
        // Notificar al modelo que ha cambiado - EVENTOS MÚLTIPLES PARA GARANTIZAR PERSISTENCIA
        try {
          // Eventos principales para notificar cambios
          eventBus.fire('element.changed', { element: element });
          eventBus.fire('elements.changed', { elements: [element] });
          eventBus.fire('bpmn.element.changed', { element: element });
          
          // Forzar que el modelo reconozca los cambios
          if (bpmnjs && bpmnjs.get) {
            try {
              var modeling = bpmnjs.get('modeling');
              var commandStack = bpmnjs.get('commandStack');
              
              // Forzar que commandStack reconozca un cambio
              if (commandStack && commandStack._stack) {
                // Marcar como "dirty" para forzar guardado
                commandStack._stack.push({
                  command: 'element.updateProperties',
                  context: {
                    element: element,
                    properties: {},
                    changed: ['extensionElements']
                  }
                });
              }
            } catch (modelError) {
              console.log('Modeling error (non-critical):', modelError);
            }
          }
          
          // Forzar la actualización del modelo a nivel más profundo
          if (element.businessObject && element.businessObject.$model) {
            element.businessObject.$model.emit('element.changed', { element: element });
          }
          
          // Marcar el diagrama como modificado
          if (typeof window !== 'undefined' && window.bpmnjs) {
            try {
              var canvas = window.bpmnjs.get('canvas');
              if (canvas) {
                canvas._svg.dispatchEvent(new Event('change'));
              }
            } catch (canvasError) {
              console.log('Canvas error (non-critical):', canvasError);
            }
          }
          
        } catch (eventError) {
          console.log('Event bus error (non-critical):', eventError);
        }
        
        // Verificar que se guardó correctamente
        setTimeout(function() {
          console.log('Verifying saved data...');
          var savedData = loadFieldsFromBPMN();
          if (savedData) {
        ...
        console.error('Error saving fields to BPMN:', e);
      }
    }
    
    function loadFieldsFromBPMN() {
      try {
        var bo = element.businessObject;
        
        // Verificar que el elemento tenga documentación
        if (!bo) {
          console.log('No business object found');
          return false;
        }
        ...
        console.log('Checking documentation array:', docs);
        
        for (var i = 0; i < docs.length; i++) {
          var doc = docs[i];
          console.log('Documentation element:', doc);
          
          // Verificar que la documentación tenga texto y contenga dataFields
          if (doc && doc.text && doc.text.includes('dataFields:')) {
            console.log('Found bpmn:Documentation with dataFields:', doc.text);
            
            try {
              var jsonText = doc.text.substring(doc.text.indexOf('dataFields:') + 'dataFields:'.length);
              var parsedData = JSON.parse(jsonText);
              console.log('Fields loaded from BPMN model:', parsedData);
              ...
              console.error('Error parsing JSON from BPMN documentation:', parseError);
            }
          }
        }
        
        console.log('No dataFields found in documentation');
        return false; // Indicar que no se encontraron campos
        
      } catch (e) {
        console.error('Error loading fields from BPMN:', e);
        return false; // Indicar que hubo un error
      }
    }

    function hydrateFieldsFromEditor() {
      // Primero intentar cargar desde el modelo BPMN
      loadFieldsFromBPMN();
      
      // Si no hay campos en el modelo BPMN, intentar cargar desde el editor JSON
      var currentText = ($editor.text() || '').trim();
      if (currentText) {
        var parsed = tryParseJSON(currentText);
        if (parsed && parsed.messageStructure && parsed.messageStructure.children) {
          // Solo cargar si no se cargaron desde el modelo BPMN
          if ($fields.find('[data-row]').length === 0) {
            parsed.messageStructure.children.forEach(function(item) {
    ...
    if ($saveBtn && $saveBtn.length) {
      $saveBtn.on('click', function() {
        console.log('🔥 Save button clicked - FORCING persistence!');
        var json = serializeFieldsToJSON();
        $editor.text(stringifyPretty(json));
          ...
          try {
            if (bpmnjs && bpmnjs.get) {
              ...
              if (eventBus) {
                ...
                eventBus.fire('diagram.saved');
              }
            }
          } catch (e) {
            console.log('Global save events error (non-critical):', e);
          }
        }, 50);
    ...
    // Botón de prueba para verificar el guardado BPMN
    var $testBtn = $overlay.find('[data-test-bpmn]');
    if ($testBtn && $testBtn.length) {
      $testBtn.on('click', function() {
        console.log('=== TESTING BPMN SAVE ===');
        var json = serializeFieldsToJSON();
        ...
        setTimeout(function() {
          console.log('=== TESTING BPMN LOAD ===');
          var loaded = loadFieldsFromBPMN();
    ...
    if ($clearBtn && $clearBtn.length) {
      $clearBtn.on('click', function() {
        ...
        // Limpiar también del modelo BPMN
        try {
          var bo = element.businessObject;
          var docs = bo.get('documentation') || [];
          ...
          for (var i = docs.length - 1; i >= 0; i--) {
            var doc = docs[i];
            if (doc && doc.text && doc.text.includes('dataFields:')) {
              // Remover la documentación con dataFields
              docs.splice(i, 1);
              console.log('Fields cleared from BPMN documentation');
            }
          }
          
          // Actualizar la documentación usando modeling
          if (bpmnjs && bpmnjs.get) {
            var modeling = bpmnjs.get('modeling');
            modeling.updateProperties(element, {
              documentation: docs
          ...
          console.error('Error clearing fields from BPMN:', e);
        }
      });
    }

    // Inicializa los campos visuales desde el modelo BPMN o JSON
    hydrateFieldsFromEditor();
    
    // Escuchar cambios en el elemento para recargar campos si es necesario
    eventBus.on('bpmn.element.changed', function(event) {
      if (event.element === element) {
        console.log('Element changed, reloading fields...');
        setTimeout(function() {
          hydrateFieldsFromEditor();
        }, 100);
      }
    ...
    // LISTENER PARA FORZAR GUARDADO ANTES DE GUARDAR EL ARCHIVO
    eventBus.on('saveXML.start', function() {
      console.log('🔥 Save XML starting - forcing field persistence for all elements!');
      try {
        // Guardar datos del elemento actual si tiene campos
        if ($fields.find('[data-row]').length > 0) {
          var json = serializeFieldsToJSON();
          if (json && json.messageStructure && json.messageStructure.children.length > 0) {
            console.log('Forcing save for current element before XML export');
            saveFieldsToBPMN(json);
          }
        }
      } catch (e) {
        console.error('Error in saveXML.start listener:', e);
      }
    });
    
    // LISTENER ADICIONAL PARA DETECTAR CTRL+S
    if (typeof document !== 'undefined') {
      document.addEventListener('keydown', function(e) {
        if ((e.ctrlKey || e.metaKey) && e.key === 's') {
          console.log('🔥 Ctrl+S detected - forcing field persistence!');
          setTimeout(function() {
            try {
              if ($fields.find('[data-row]').length > 0) {
                var json = serializeFieldsToJSON();
                if (json && json.messageStructure && json.messageStructure.children.length > 0) {
                  console.log('Forcing save due to Ctrl+S');
                  saveFieldsToBPMN(json);
                }
              }
            } catch (e) {
              console.error('Error in Ctrl+S handler:', e);
            }
          }, 10); // Inmediato
        }
      });
  ...
  // FUNCIÓN GLOBAL PARA FORZAR GUARDADO DE TODOS LOS ELEMENTOS CON DATOS
  this.forceGlobalSave = function() {
    console.log('🔥 FORCING GLOBAL SAVE OF ALL DATA FIELDS');
    try {
      ...
      allOverlays.forEach(function(overlay) {
        try {
          ...
          var $fields = $overlay.find('[data-fields]');
          ...
          if ($fields.find('[data-row]').length > 0) {
            ...
            var json = {
              ...
              pool: "Unknown Pool", // Simplificado por ahora
              name: "New CI",
              type: "Communicative Interaction",
              messageStructure: {
                name: "Default Name",
            ...
            $fields.find('[data-row]').each(function() {
              ...
              json.messageStructure.children.push(item);
            });
            
            if (json.messageStructure.children.length > 0) {
              // Usar el método oficial de guardado
              var bo = element.businessObject;
              if (bpmnjs && bpmnjs.get) {
                try {
                  var modeling = bpmnjs.get('modeling');
                  ...
                  if (!existingDoc) {
                    var newDoc = bo.$model.create('bpmn:Documentation', {
                      textFormat: 'text/plain',
                      text: 'dataFields:' + JSON.stringify(json)
                    });
                    docs.push(newDoc);
                  } else {
                    existingDoc.text = 'dataFields:' + JSON.stringify(json);
                  }
                  ...
                  console.error('Error saving element data:', e);
                }
              }
            }
          }
      ...
      console.log('✅ Global save completed! Saved data for', savedCount, 'elements');
      return savedCount;
      
    } catch (e) {
      console.error('Error in global save:', e);
      return 0;
    }
  };
  ...
  if (typeof window !== 'undefined') {
    window.forceGlobalSave = this.forceGlobalSave;
  }
}
...
},{"./util":5,"jquery":7,"lodash":8}],4:[function(require,module,exports){
module.exports = {
  __init__: [ 'comments' ],
  'comments': [ 'type', require('./comments') ]
};
},{"./comments":3}],5:[function(require,module,exports){
